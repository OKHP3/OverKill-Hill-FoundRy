...
### 🔹 Purpose
This block defines the explicit and implicit conditions that cause ArcSyntrixo‑Rᵧ to activate its casting logic. Triggers can be external (user uploads, ledger call) or internal (overlay collapse, PME drift). ArcSyntrixo‑Rᵧ does not run passively — it awakens by condition, by contract.
