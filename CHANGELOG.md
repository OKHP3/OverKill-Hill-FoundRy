# Capability workbench — 2026-09-07

- Record owner-confirmed public visibility and reciprocal FoundRy mentoring; reconcile current governance and preserve dated historical review evidence.

- Add OverKill capability projects for prompts, skills, workflows and software starters, with local persistence, recovery backups, source preview and ZIP exports.
- Retain the Custom GPT creator at `#creator` and preserve its separate workspace data.
- Document the seven-element universe, region boundaries and current source/deployment evidence.
- Preserve existing private-source graduation controls and record the repository visibility discrepancy.

# Changelog

All notable changes to the private OverKill Hill FoundRy relay and its hosted capabilities should be recorded here. ReFolDec entries describe the FoundRy-hosted capability scaffold unless they explicitly identify a separately approved public release.

## 0.1.0 — ReFolDec hosted capability scaffold

- Established ReFolDec as a FoundRy-hosted capability for the Recursively Folding Codec.
- Defined the fold / unfold / refold operating model.
- Positioned xME/xIE/xMIE as underlying mechanics rather than competing names.
- Added publication-boundary guidance: Notion and the FoundRy relay are private; a public ReFolDec artifact requires separate graduation.
- Recorded ReFolDec-specific specifications, roadmap, manifest, schema, vocabulary, and example materials without changing the FoundRy relay identity.
- Added initial specification, roadmap, manifest, schema, vocabulary, and example materials.
