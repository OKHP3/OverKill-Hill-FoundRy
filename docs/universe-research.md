...
There are three different relationships to preserve: **brand and product ownership**, **governance inheritance**, and **distribution of reusable skills**. They are related but are not interchangeable. Universal governance originates in OverKill-Hill. The owner subsequently clarified that OverKill-Hill is also the baseline pattern the other sites borrow from and defer to on questions. This FoundRy similarly mentors the two sibling FoundRys, with improvements able to flow back to the mentor or across to the other sibling; see the [mentoring model](foundry-mentoring-model.md). Sharing a skill does not transfer ownership of a sibling's projects or authorize disclosure of its private source material. This interpretation follows the owner's September 7 instruction; the public [universe page](https://overkillhill.com/universe/) supports distinct site roles but is a navigation map rather than an authoritative access-control diagram.
...
|---|---|---|
| At discovery, the repository was public while root README, AGENTS and manifest described private posture. | Live GitHub metadata on September 7; local governance files; public Pages creator. | Resolved by owner clarification on September 7: public status is intentional. Current charter and manifest are reconciled; private-source graduation rules still apply. |
| Some older lineage fields call this FoundRy a parent of sibling FoundRys. | Sibling manifests and governance guidance; OverKill-Hill's Tier 0 / Tier 1 model. | Owner clarification establishes this FoundRy as the baseline mentor, with reciprocal learning among all three FoundRys. This relationship does not transfer application ownership. Universal rules are reconciled at OverKill-Hill. |
